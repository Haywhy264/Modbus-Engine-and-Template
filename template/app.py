...
st.set_page_config(page_title="Table Variable CSV Builder", layout="wide")
...
PAGE_CONFIGS = {
    ...
    "MCLHYST_Tags": {
        ...
        # Headers encode two parallel logical columns per cell, separated by /
        "headers": [
            "Control Tag/Enable",
            "Name/Input Low",
            "Unit/Output Low",
    ...
    "MCLHYSTDYN_Tags": {
        ...
        "float_columns": {"Start", "End", "Step Size", "Default Output"} | {f"Output Step {i}" for i in range(1, 55)},
        "boolean_columns": set(),
        "interval_columns": {"Interval"},
        "required_text_columns": {"Control Tag", "Name"},
        "ipv4_columns": set(),
    ...
    # Key-value config file; represented as a flat single-row table for consistent UX
    "JSON_TCP_Servers": {
    ...
    "MCLPROP_Tags": {
        ...
        "headers": [
            "Control Tag", "Name", "Unit", "Input Tag", "Source Tag",
            "Source Enable Value", "Default Enable", "Default Value", "Interval",
            "P1 Enable", "P1 Input", "P1 Output",
            "P2 Enable", "P2 Input", "P2 Output",
            "P3 Enable", "P3 Input", "P3 Output",
            "P4 Enable", "P4 Input", "P4 Output",
            "P5 Enable", "P5 Input", "P5 Output",
            "P6 Enable", "P6 Input", "P6 Output",
        ],
        "integer_columns": set(),
        "float_columns": {
            "Source Enable Value", "Default Value",
            "P1 Input", "P1 Output", "P2 Input", "P2 Output",
            "P3 Input", "P3 Output", "P4 Input", "P4 Output",
            "P5 Input", "P5 Output", "P6 Input", "P6 Output",
        },
        "boolean_columns": {
            "Default Enable",
            "P1 Enable", "P2 Enable", "P3 Enable",
            "P4 Enable", "P5 Enable", "P6 Enable",
        },
        "interval_columns": {"Interval"},
        "required_text_columns": {"Control Tag", "Name"},
        "ipv4_columns": set(),
    ...
    "MVBOO_Tags": {
        ...
        "headers": ["Tag", "Name", "Unit", "Input Tag", "Source Tag", "Source Enable Value", "Interval"],
        "integer_columns": set(),
    ...
    # Key-value config file adapted to a single-row flat table; original format has no header row
    "MVJSON_Directory": {
    ...
    # 15 child column groups repeat; headers use pandas .1/.14 suffix disambiguation
    "MVSENSOR_Tags": {
        "title": "MVSENSOR Tags CSV Builder",
        "headers": [
            "Tag", "Name", "Unit", "Interval", "Delta", "Limits",
            "Lower Limit Value", "Upper Limit Value", "Lower Limit Tag", "Upper Limit Tag",
        ] + [
            name if i == 0 else f"{name}.{i}"
            for i in range(15)
            for name in ["Child Tag", "Child Include", "Child Multiplier", "Child Multiplier Tag", "Child Offset"]
        ],
    ...
    "MCLIPOP_Tags": {
        ...
        "headers": [
            "Control Tag", "Name", "Unit", "Input Tag", "Control Enable",
            "Repeat Mode", "Interval", "Multiplier", "Offset",
            "Minimum Limit Enable", "Minimum Limit",
            "Maximum Limit Enable", "Maximum Limit",
        ],
    ...
    for idx, row in dataframe.iterrows():
        ...
        for column in boolean_columns:
            ...
            if value not in BOOL_VALUES:
                row_errors.append(
                    f"{column}: value must be one of {sorted(BOOL_VALUES)}"
                )
        ...
        for column in interval_columns:
            ...
            if not INTERVAL_PATTERN.fullmatch(value):
                row_errors.append(
                    f"{column}: '{value}' must match pattern T#<number><ms|s|m|h>"
                )
...
def run_csv_page(page_key: str) -> None:
    config = PAGE_CONFIGS[page_key]
    headers: List[str] = config["headers"]
    default_filename: str = config["default_filename"]
    state_key = f"table_df_{page_key}"

    st.title(config["title"])
    st.caption(
        "Upload and edit table variables with strict checks, then export a CSV in the same structure."
    )

    if page_key == "Plant_Node":
        st.info("IP Address examples: 192.168.1.11, 10.0.0.5")
    elif page_key == "JSON_TCP_Servers":
        st.info(
            "Each row is one server config. "
            "Original file uses a key-value format; this page uses standard CSV columns for consistent editing."
        )
    elif page_key == "MCLHYST_Tags":
        st.info(
            "Column headers use A/B notation representing two paired logical fields per column."
        )
    elif page_key in {"MVJSON_Directory"}:
        st.info(
            "Original file is a two-row key-value config. "
            "Represented here as a single-row flat table for consistent editing."
        )
    elif page_key == "MVSENSOR_Tags":
        st.info(
            "The 15 repeated Child column groups are disambiguated with .1–.14 suffixes "
            "(matching how pandas renames duplicate headers on upload)."
        )
    elif page_key == "MVJSON_Tags":
        st.info("Source IP is validated as a strict IPv4 address (e.g. 192.168.1.10).")

    with st.sidebar:
        st.subheader("Format Rules")
        st.write("- Headers are locked to the selected template")
        st.write(
            "- Validation is strict for integer, float, boolean, interval, and required text fields"
        )
        st.write("- Plant_Node and MVJSON_Tags validate IP Address / Source IP as strict IPv4")
        st.write(f"- Default filename: {default_filename}")
    ...
    if uploaded_file is not None:
        try:
            ...
            if incoming_headers != headers:
                st.error(
                    "Header mismatch: uploaded CSV must use exactly this header order: "
                    + ", ".join(headers)
                )
            else:
                ...
                st.success("CSV loaded and validated against template headers.")
    ...
    with action_col2:
        # Only pages with an Index column support auto-numbering
        if "Index" in headers:
            if st.button("Auto-number Index (1..N)", key=f"autonum_btn_{page_key}"):
    ...
    if is_valid:
        ...
        st.warning("Fix validation issues before downloading.")
    ...
    timestamped_name = f"{file_stem}_{timestamp_suffix}.csv"
