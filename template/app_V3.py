...
st.set_page_config(page_title="Table Variable CSV Builder - Version 3", layout="wide")
...
def parse_key_value_csv(uploaded_file: io.BytesIO, headers: List[str]) -> pd.DataFrame:
    uploaded_file.seek(0)
    raw_df = pd.read_csv(uploaded_file, header=None, dtype="string").fillna("")
    values: Dict[str, str] = {}
...
def parse_uploaded_template(uploaded_file: io.BytesIO, page_key: str, headers: List[str]) -> pd.DataFrame:
    if page_key in {"JSON_TCP_Servers", "MVJSON_Directory"}:
        return parse_key_value_csv(uploaded_file, headers)
    ...
    if incoming_headers != headers:
        raise ValueError("Header mismatch. Expected: " + ", ".join(headers))
...
def render_page_editor(page_key: str) -> None:
    ...
    uploaded_file = st.file_uploader(
        f"Upload source CSV ({default_filename})",
        type=["csv"],
...
with st.sidebar:
    st.subheader("Version3")
    selected_template_name = st.selectbox("Select CSV Template", options=TEMPLATE_NAMES, index=0)
