...
st.set_page_config(
    page_title="Site Configuration Tool - Version 6",
    layout="wide",
    page_icon="⚙️",
)
...
logo_path = os.path.join(os.path.dirname(__file__), "assets", "Gridbeyond logo.jpg")
...
with header_col2:
    st.markdown(
        "<div style='font-size: 4.2rem; font-weight: 700; letter-spacing: -0.04em; color: #2d3d7b; margin-top: 0.8rem;'>Site Configuration Tool</div>",
...
def validate_all_templates() -> Tuple[bool, Dict[str, Dict[int, List[str]]]]:
    ...
    for page_key, config in PAGE_CONFIGS.items():
        ...
        is_ignored = page_key in st.session_state.get("ignored_validation_templates", set())
        if is_ignored:
            continue
...
class ControllerSSHBridge:
    """Handles SSH connection and SFTP/SCP directory transfers for config sync."""
...
if "ignored_validation_templates" not in st.session_state:
    st.session_state["ignored_validation_templates"] = set()
...
def get_template_validation_state(page_key: str) -> Tuple[bool, Dict[int, List[str]]]:
    config = PAGE_CONFIGS[page_key]
    state_key = get_state_key(page_key)
    df = to_text_dataframe(st.session_state[state_key])
...
def is_template_effectively_valid(page_key: str) -> bool:
    state_key = get_state_key(page_key)
    row_count = len(st.session_state.get(state_key, pd.DataFrame(columns=PAGE_CONFIGS[page_key]["headers"])))

    if row_count == 0:
        return True

    is_valid, _ = get_template_validation_state(page_key)
    is_ignored = page_key in st.session_state.get("ignored_validation_templates", set())
    return is_valid or is_ignored


def build_template_status_dropdown_options() -> Tuple[List[str], Dict[str, str]]:
    options: List[str] = []
    value_map: Dict[str, str] = {}

    for page_key in TEMPLATE_KEYS:
        filename = PAGE_CONFIGS[page_key]["default_filename"]
        state_key = get_state_key(page_key)
        row_count = len(st.session_state.get(state_key, pd.DataFrame(columns=PAGE_CONFIGS[page_key]["headers"])))

        if row_count == 0:
            label = f"{filename} - ❌ Not uploaded"
        else:
            effective_valid = is_template_effectively_valid(page_key)
            label = f"{filename} - {'✅ Validated' if effective_valid else '❌ Needs validation'}"

        options.append(label)
        value_map[label] = filename
...
def render_page_editor(page_key: str) -> None:
    ...
    is_valid, validation_errors = get_template_validation_state(page_key)
    is_ignored = page_key in st.session_state.get("ignored_validation_templates", set())

    st.markdown("#### Validation Status")
    if is_valid or is_ignored:
        ...
        ignore_key = f"ignore_validation_{page_key}"
        if st.button(
            "Ignore validation requirement for this file",
            key=ignore_key,
            help="This keeps the validation check active while suppressing the blocking requirement for this session.",
        ):
            ignored_set = st.session_state.setdefault("ignored_validation_templates", set())
            if page_key in ignored_set:
                ignored_set.remove(page_key)
            else:
                ignored_set.add(page_key)
            st.rerun()

        if is_ignored:
            st.info("ℹ️ Ignored status is active for this file; validation remains active and the file is still checked.")
    ...
    allow_download = is_valid or is_ignored
...
with st.sidebar:
    st.caption("Version 6 • Box-to-Box SSH/SCP & Template Editor")
    ...
    if app_mode == "📝 Single Template Editor":
        ...
        template_status_options, template_status_map = build_template_status_dropdown_options()
        ...
        selected_template_name = template_status_map[selected_template_label]
        selected_page = TEMPLATE_NAME_TO_KEY[selected_template_name]
    else:
        st.subheader("Quick Switch to Editor")
        template_status_options, template_status_map = build_template_status_dropdown_options()
        quick_template_label = st.selectbox(
            "Jump to Template CSV",
            options=template_status_options,
            index=0,
            key="sb_quick_template_select",
        )
        quick_template_name = template_status_map[quick_template_label]
