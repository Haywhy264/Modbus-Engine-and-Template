#!/usr/bin/env python3
"""
Modbus TCP dummy sensor simulator for SiteSee2 automated testing (Version 4).

Version 4 additions
-------------------
- Simulates 50 dummy sensor tags together in one update cycle
- Uses 50 contiguous holding registers (FC03), mirrored to input registers (FC04)
- Keeps V3 external-client behavior (0.0.0.0 default bind + connected IP tracking)

Register map
------------
- Tags: sensor_01 ... sensor_50
- Address range: 0..49
...
class DummyTagGenerator:
    """Produces deterministic, repeatable values for 50 dummy sensor tags."""
...
class MeterSimulator:
    """50-tag Modbus TCP dummy sensor simulator (Version 4)."""
    ...
    async def start(self) -> None:
        ...
        identity.VendorName = "SiteSee2 Sensor Simulator V4"
        identity.ProductCode = "SIM-SENSOR-4"
        identity.VendorUrl = "http://sitesee2.local"
        identity.ProductName = "Dummy Sensor Simulator V4"
        identity.ModelName = "SIM-SENSOR-4"
        identity.MajorMinorRevision = "4.0.0"
        ...
        logger.info(
            "Sensor simulator V4 listening on %s:%d  unit_id=%d  tags=%d  update_interval=%.0f ms  seed=%d",
            self.host,
            self.port,
            self.unit_id,
            SENSOR_COUNT,
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Modbus TCP 50-tag dummy sensor simulator V4 for SiteSee2 testing",
