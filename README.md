...
# V4 full help
& ".\.venv\Scripts\python.exe" ".\modbus_meter_simulator_v4.py" --help

# V5 full help
& ".\.venv\Scripts\python.exe" ".\modbus_meter_simulator_v5.py" --help
```
...
## V4 Execution (50 Dummy Sensor Tags)

`modbus_meter_simulator_v4.py` provides a 50-tag dummy sensor profile for bulk tag polling.

### Files

- `modbus_meter_simulator_v4.py`
- `modbus_simulator_v4_gui.py`
- `run_modbus_simulator_v4.bat`
- `run_modbus_simulator_v4_gui.bat`

### V4 register model

- Tag count: `50`
- Register count: `50` contiguous words
- Address range: `0..49`
- Tag names: `sensor_01` to `sensor_50`
- Data type: `uint16`
- Read functions: `FC03` (holding) and `FC04` (input mirror)

### Run V4 simulator directly

```powershell
& ".\.venv\Scripts\python.exe" ".\modbus_meter_simulator_v4.py" --port 5022 --display-interval 1
```

### Run V4 simulator via batch launcher
...
```powershell
& ".\.venv\Scripts\python.exe" ".\modbus_simulator_v4_gui.py"
```

### Polling all V4 tags together

- Configure your Modbus client to read `count=50` starting at address `0`.
- This returns all V4 dummy sensor tags in one request cycle.

---

## V5 Execution (500 Dummy Sensor Tags)

`modbus_meter_simulator_v5.py` provides a 500-tag dummy sensor profile for high-density bulk polling.
...
- `Plant_tag.csv` (500 tag definitions for import/mapping)

### V5 register model

- Tag count: `500`
- Register count: `500` contiguous words
- Address range: `0..499`
- Tag names: `sensor_001` to `sensor_500`
- Data type: `uint16`
- Read functions: `FC03` (holding) and `FC04` (input mirror)

### Run V5 simulator directly

```powershell
& ".\.venv\Scripts\python.exe" ".\modbus_meter_simulator_v5.py" --port 5023 --display-interval 1
```

### Run V5 simulator via batch launcher
...
```powershell
& ".\.venv\Scripts\python.exe" ".\modbus_simulator_v5_gui.py"
```

### Polling all V5 tags together

- Configure your Modbus client to read `count=500` starting at address `0`.
- This returns all V5 dummy sensor tags in one request cycle.
...
- Added `modbus_meter_simulator_v4.py` with 50 dummy sensor tags on addresses `0..49`
- Added `modbus_simulator_v4_gui.py` and V4 launchers (`run_modbus_simulator_v4*.bat`)
- Added `modbus_meter_simulator_v5.py` with 500 dummy sensor tags on addresses `0..499`
- Added `modbus_simulator_v5_gui.py` and V5 launchers (`run_modbus_simulator_v5*.bat`)
- Added `Plant_tag.csv` with `sensor_001..sensor_500` tag mappings
